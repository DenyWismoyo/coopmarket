...
export default function POSPage() {
          ...
          const groups = cart.reduce((acc, item) => {
            if (!acc[item.sellerId]) {
              acc[item.sellerId] = { 
                 ...
                 sellerType: item.sellerType || 'member', 
                 amount: 0, 
                 qrisUrl: null 
               };
            }
            acc[item.sellerId].amount += (item.price * item.qty);
            return acc;
  ...
  }, [isCheckoutOpen, paymentType, cart, userData]);
        ...
        {cart.length === 0 ? (
            ...
            <>
                <ScrollArea className="flex-1 p-2 sm:p-3">
                    ...
                    {cart.map((item) => {
                        // [PERBAIKAN] Dukungan fallback jika menggunakan struktur data lama imageUrl
                        const imgUrl = item.images?.[0] || item.imageUrl;
                        return (
                        <div key={item.cartId} className="flex gap-2 bg-white p-2 rounded-xl border border-zinc-200 shadow-sm transition-all items-center">
  ...
  return (
    <div className={cn(
       ...
       // [PERBAIKAN] Mengubah z-[100] menjadi z-[40] agar popup (z-50) bisa muncul di atasnya
       isFullscreen 
                  ...
                  <CardContent className="p-3 flex flex-col flex-1 justify-between gap-2">
                    ...
                    <div className="pt-2 border-t border-zinc-100/80">
                        ...
                        {product.sellerType === 'member' && (
                            <p className="text-[9px] text-zinc-400 mt-1.5 truncate font-medium">
                                <User className="w-2.5 h-2.5 inline mr-0.5 text-blue-400" /> {product.sellerName}
                            </p>
                        )}
                    </div>
                  </CardContent>
      ...
      <div className="hidden lg:flex w-[340px] xl:w-[380px] flex-col bg-white rounded-2xl border border-zinc-200 shadow-xl h-full print:hidden relative overflow-hidden">
        <div className="p-3.5 border-b border-zinc-100 bg-zinc-50 flex justify-between items-center z-10 shadow-sm">
          <h2 className="font-bold text-sm flex items-center gap-2 text-zinc-800">
            <ShoppingCart className="w-4 h-4 text-blue-600" /> Keranjang Belanja
          </h2>
          {cart.length > 0 && (
          ...
          <div className="space-y-5 py-2">
                ...
                {selectedMember ? (
                    <div className="flex items-center justify-between p-3 bg-blue-50 border border-blue-200 rounded-xl">
                        <div className="flex items-center gap-3">
                            ...
                            <div>
                                <p className="text-sm font-bold text-blue-900">{selectedMember.fullName}</p>
                                <p className="text-[10px] text-blue-600 font-mono">ID: {selectedMember.uid.slice(0,8)}</p>
                            </div>
                        </div>
                        <button onClick={handleClearMember} className="text-zinc-400 hover:text-red-500 p-2 bg-white rounded-full shadow-sm">
                            <X className="w-4 h-4" />
                        </button>
                    </div>
                ) : (
                    <div className="relative">
                        ...
                        <Input 
                            ...
                            placeholder="Cari member / Ketik nama pelanggan..." 
                             className="pl-9 h-11 border-zinc-300 rounded-xl" 
                         />
                        
                        {memberSearchQuery.length > 2 && memberResults.length > 0 && (
                            <div className="absolute top-full left-0 right-0 mt-2 bg-white border border-zinc-200 rounded-xl shadow-xl z-10 max-h-48 overflow-y-auto p-1">
                                {memberResults.map(m => (
                                    <button 
                                        key={m.uid}
                                        className="w-full text-left px-4 py-3 hover:bg-zinc-50 rounded-lg flex flex-col gap-0.5 transition-colors"
                                        onClick={() => handleSelectMember(m)}
                                    >
                                        <p className="text-sm font-bold text-zinc-900">{m.fullName}</p>
                                        <p className="text-[10px] text-zinc-500">{m.phone || m.email}</p>
                                    </button>
                                ))}
                            </div>
                        )}
             ...
             {paymentType === 'cash' ? (
                ...
                <div className="space-y-4 animate-in fade-in slide-in-from-bottom-2 duration-200 max-h-[50vh] overflow-y-auto pr-2 scrollbar-thin">
                  {isFetchingQris ? (
                    <div className="flex flex-col items-center justify-center py-10 bg-zinc-50 rounded-xl">
                      <Loader2 className="w-8 h-8 animate-spin text-blue-600 mb-3" />
                      <p className="text-sm font-medium text-zinc-500">Menyiapkan Barcode QRIS...</p>
                    </div>
                  ) : (
                    <div className="space-y-4">
                      <div className="border border-blue-200 rounded-2xl p-5 bg-blue-50/50 flex flex-col items-center shadow-sm">
                         ...
                         {coopQrisUrl ? (
                            <>
                               <div 
                                   className="w-48 h-48 relative rounded-2xl overflow-hidden border-2 border-dashed border-blue-400 bg-white p-2 cursor-pointer group hover:border-blue-600 transition-colors shadow-lg"
                                  onClick={() => setZoomedImage(coopQrisUrl)}
                               >
                               ...
                               <p className="text-[10px] font-bold text-zinc-400 mt-3 flex items-center gap-1.5 uppercase tracking-wider">
                                  Klik untuk memperbesar <ZoomIn className="w-3 h-3" />
                               </p>
                            </>
                         ) : (
                            <div className="text-center p-6 bg-white/80 text-red-500 rounded-2xl w-full border border-red-100 shadow-sm">
                               <QrCode className="w-10 h-10 mx-auto mb-2 opacity-50" />
                               <p className="text-sm font-bold">QRIS Unit belum diatur</p>
                            </div>
                         )}
                      </div>
                           ...
                           <div className="grid grid-cols-2 gap-3">
                               {qrisData.filter(d => d.sellerType !== 'coop').map((data, idx) => (
                                  <div key={idx} className="border border-zinc-200 rounded-2xl p-3 bg-white flex flex-col items-center shadow-sm">
                                     ...
                                     {data.qrisUrl ? (
                                        <div 
                                             className="w-full aspect-square relative rounded-xl overflow-hidden border border-zinc-200 bg-white p-1 cursor-pointer group hover:border-blue-400 transition-colors"
                                            onClick={() => setZoomedImage(data.qrisUrl!)}
                                        >
                                        ...
                                        <div className="text-center p-3 bg-red-50 text-red-500 rounded-xl w-full border border-red-100 h-full flex flex-col items-center justify-center">
                                           <QrCode className="w-6 h-6 mx-auto mb-1 opacity-50" />
                                           <p className="text-[9px] font-bold leading-tight">Belum diatur</p>
                                        </div>
                                     )}
                                  </div>
                               ))}
          ...
          <DialogFooter className="pt-4 border-t border-zinc-100 mt-2 gap-2 sm:gap-0">
            ...
            <Button 
                 ...
                 disabled={
                   ...
                   (paymentType === 'qris' && !coopQrisUrl)
                 } 
                 className="h-12 rounded-xl bg-blue-600 hover:bg-blue-700 font-bold text-base px-8"
            >
               {processing ? <Loader2 className="w-5 h-5 animate-spin" /> : "Proses Pembayaran"}
            </Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>

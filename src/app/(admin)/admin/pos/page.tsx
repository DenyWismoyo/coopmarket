...
import { 
    ...
    Loader2, Package, RefreshCw, User, ChevronUp, X, 
    UserCheck, QrCode, ZoomIn, Maximize, Minimize, 
    ScanBarcode, Trash 
} from "lucide-react";
...
export default function POSPage() {
  ...
  const [qrisData, setQrisData] = useState<{sellerId: string, sellerName: string, sellerType: string, amount: number, qrisUrl: string | null}[]>([]);
  const [coopQrisUrl, setCoopQrisUrl] = useState<string | null>(null); 
  const [isFetchingQris, setIsFetchingQris] = useState(false);
  ...
  useEffect(() => { fetchProducts(); }, [userData]);

  useEffect(() => {
    let result = products;
    if (search) {
        const lower = search.toLowerCase();
        result = result.filter(p => p.name.toLowerCase().includes(lower) || p.sellerName?.toLowerCase().includes(lower) || p.id.toLowerCase().includes(lower));
    }
  ...
  useEffect(() => {
    if (isCheckoutOpen && paymentType === 'qris') {
      const fetchQris = async () => {
        setIsFetchingQris(true);
        try {
          if (userData?.coopId) {
             const coopProfile = await cooperativeService.getCooperativeById(userData.coopId);
             setCoopQrisUrl(coopProfile?.qrisUrl || null);
          }
          const groups = cart.reduce((acc, item) => {
            if (!acc[item.sellerId]) {
              acc[item.sellerId] = { sellerId: item.sellerId, sellerName: item.sellerName, sellerType: item.sellerType || 'member', amount: 0, qrisUrl: null };
            }
            acc[item.sellerId].amount += (getEffectivePrice(item) * item.qty);
            return acc;
          }, {} as Record<string, {sellerId: string, sellerName: string, sellerType: string, amount: number, qrisUrl: string | null}>);

          const results = [];
          for (const sellerId in groups) {
            const group = groups[sellerId];
            if (group.sellerType === 'coop') {
              const coop = await cooperativeService.getCooperativeById(sellerId);
              results.push({ ...group, qrisUrl: coop?.qrisUrl || null });
            } else {
              const userProfile = await authService.getUserProfile(sellerId);
              results.push({ ...group, qrisUrl: userProfile?.qrisUrl || null });
            }
          }
          setQrisData(results);
        } catch (error) {
  ...
  }, [isCheckoutOpen, paymentType, cart, userData, selectedMember]);
  ...
  const handleCheckout = async () => {
    ...
    try {
      const orderItems = cart.map(item => ({
        ...
        variant: item.selectedVariant ? { id: item.selectedVariant.id, name: item.selectedVariant.name, price: getEffectivePrice(item) } : undefined
      }));
      ...
      const result = await orderService.createOrder({
        ...
        status: "completed",
        isOffline: true,
        notes: paymentType === 'qris' ? "Transaksi Kasir (QRIS)" : "Transaksi Kasir (Tunai)"
      });
  ...
  // PENTING: Perubahan CartContent menjadi fungsi regular (bukan komponen) agar tidak hilang fokus saat mengetik
    ...
    <div className={cn("flex flex-col lg:flex-row gap-4 lg:gap-6 transition-all duration-300", isFullscreen ? "fixed inset-0 z-50 bg-zinc-50 p-4 h-screen" : "h-[calc(100vh-6rem)] pb-20 lg:pb-0")}>
      ...
      <div className="hidden lg:flex w-[340px] xl:w-[380px] flex-col bg-white rounded-2xl border border-zinc-200 shadow-xl h-full print:hidden relative overflow-hidden">
        <div className="p-3.5 border-b border-zinc-100 bg-zinc-50 flex justify-between items-center z-10 shadow-sm">
          <h2 className="font-bold text-sm flex items-center gap-2 text-zinc-800"><ShoppingCart className="w-4 h-4 text-blue-600" /> Keranjang Belanja</h2>
        ...
        {/* PENGGUNAAN FUNGSI RENDER DI SINI */}
        <div className="flex-1 overflow-hidden flex flex-col bg-zinc-50/30 relative">{renderCartContent()}</div>
      </div>
      ...
      <div className="lg:hidden fixed bottom-0 left-0 right-0 z-[45] bg-white border-t p-4 shadow-[0_-10px_40px_rgba(0,0,0,0.1)] pb-safe">
         <Sheet open={isMobileCartOpen} onOpenChange={setIsMobileCartOpen}>
            ...
            <SheetContent side="bottom" className="h-[85vh] flex flex-col p-0 rounded-t-[2rem] overflow-hidden">
                ...
                {/* PENGGUNAAN FUNGSI RENDER DI SINI */}
                <div className="flex-1 overflow-hidden flex flex-col bg-white">{renderCartContent()}</div>
            </SheetContent>
         </Sheet>
      </div>
      ...
      <Dialog open={isCheckoutOpen} onOpenChange={(open) => { setIsCheckoutOpen(open); if (!open) setPaymentType('cash'); }}>
        <DialogContent className="sm:max-w-md rounded-2xl">
          ...
          <div className="space-y-5 py-2">
             ...
             {/* UI Identitas Pembeli Dipindahkan, Hanya Menampilkan Hasil di Sini */}
             <div className="space-y-2">
             ...
             <div className="flex bg-zinc-100 p-1.5 rounded-xl border border-zinc-200 shadow-inner">
               ...
               <button onClick={() => setPaymentType('qris')} className={`flex-1 py-2 text-sm font-bold rounded-lg transition-all ${paymentType === 'qris' ? 'bg-white shadow text-zinc-900' : 'text-zinc-500 hover:text-zinc-700'}`}>QRIS / Transfer</button>
             </div>
             ...
             {paymentType === 'cash' ? (
                ...
                <div className="space-y-4 animate-in fade-in slide-in-from-bottom-2 duration-200 max-h-[50vh] overflow-y-auto pr-2 scrollbar-thin">
                  {isFetchingQris ? (
                    <div className="flex flex-col items-center justify-center py-10 bg-zinc-50 rounded-xl"><Loader2 className="w-8 h-8 animate-spin text-blue-600 mb-3" /><p className="text-sm font-medium text-zinc-500">Menyiapkan Barcode QRIS...</p></div>
                  ) : (
                    <div className="space-y-4">
                      <div className="border border-blue-200 rounded-2xl p-5 bg-blue-50/50 flex flex-col items-center shadow-sm">
                         <div className="text-center mb-4">
                            <p className="font-black text-blue-900 text-lg tracking-tight">QRIS Unit Internal</p>
                            <p className="text-xs font-medium text-blue-700/80 mb-1">Pembayaran Terpusat</p>
                            <p className="text-sm font-bold text-blue-800 bg-white px-3 py-1 rounded-full shadow-sm inline-block">Total: {formatCurrency(totalAmount)}</p>
                         </div>
                         
                         {coopQrisUrl ? (
                            <>
                               <div className="w-48 h-48 relative rounded-2xl overflow-hidden border-2 border-dashed border-blue-400 bg-white p-2 cursor-pointer group hover:border-blue-600 transition-colors shadow-lg" onClick={() => setZoomedImage(coopQrisUrl)}>
                                  <Image src={coopQrisUrl} alt="QRIS Unit" fill className="object-contain p-2" />
                                  <div className="absolute inset-0 bg-black/60 opacity-0 group-hover:opacity-100 flex items-center justify-center transition-opacity backdrop-blur-sm"><ZoomIn className="w-10 h-10 text-white" /></div>
                               </div>
                               <p className="text-[10px] font-bold text-zinc-400 mt-3 flex items-center gap-1.5 uppercase tracking-wider">Klik untuk memperbesar <ZoomIn className="w-3 h-3" /></p>
                            </>
                         ) : (
                            <div className="text-center p-6 bg-white/80 text-red-500 rounded-2xl w-full border border-red-100 shadow-sm"><QrCode className="w-10 h-10 mx-auto mb-2 opacity-50" /><p className="text-sm font-bold">QRIS Unit belum diatur</p></div>
                         )}
                      </div>

                      {qrisData.filter(d => d.sellerType !== 'coop').length > 0 && (
                         <>
                           <div className="relative flex items-center py-2">
                              <div className="flex-grow border-t-2 border-dashed border-zinc-200"></div>
                              <span className="flex-shrink-0 mx-4 text-[10px] font-black text-zinc-400 uppercase tracking-widest bg-white px-2">Bayar Terpisah (Titipan)</span>
                              <div className="flex-grow border-t-2 border-dashed border-zinc-200"></div>
                           </div>
                           
                           <div className="grid grid-cols-2 gap-3">
                               {qrisData.filter(d => d.sellerType !== 'coop').map((data, idx) => (
                                  <div key={idx} className="border border-zinc-200 rounded-2xl p-3 bg-white flex flex-col items-center shadow-sm">
                                     <div className="text-center mb-2 w-full">
                                        <p className="font-bold text-zinc-900 text-xs truncate">{data.sellerName}</p>
                                        <p className="text-[10px] font-bold text-blue-600 mt-0.5">{formatCurrency(data.amount)}</p>
                                     </div>
                                     {data.qrisUrl ? (
                                        <div className="w-full aspect-square relative rounded-xl overflow-hidden border border-zinc-200 bg-white p-1 cursor-pointer group hover:border-blue-400 transition-colors" onClick={() => setZoomedImage(data.qrisUrl!)}>
                                           <Image src={data.qrisUrl} alt={`QRIS ${data.sellerName}`} fill className="object-contain p-1" />
                                        ...
                                        <div className="text-center p-3 bg-red-50 text-red-500 rounded-xl w-full border border-red-100 h-full flex flex-col items-center justify-center"><QrCode className="w-6 h-6 mx-auto mb-1 opacity-50" /><p className="text-[9px] font-bold leading-tight">Belum diatur</p></div>
                                     )}
                                  </div>
                               ))}
                           </div>
          ...
          <DialogFooter className="pt-4 border-t border-zinc-100 mt-2 gap-2 sm:gap-0">
            ...
            <Button onClick={handleCheckout} disabled={processing || (paymentType === 'cash' && changeAmount < 0) || (paymentType === 'qris' && !coopQrisUrl)} className="h-12 rounded-xl bg-blue-600 hover:bg-blue-700 font-bold text-base px-8">
      ...
      <ReceiptDialog open={isSuccessOpen} onOpenChange={setIsSuccessOpen} order={lastOrder} coopName={userData?.coopName || "Koperasi"} cashierName={userData?.fullName || "Admin"} />
    </div>
  );
}